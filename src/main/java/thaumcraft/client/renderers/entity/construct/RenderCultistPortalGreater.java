...
public class RenderCultistPortalGreater extends Render
{
    ...
    public void renderPortal(final EntityCultistPortalGreater portal, final double px, double py, final double pz, final float par8, final float f) {
        ...
        final float bob = MathHelper.sin(portal.ticksExisted / (5.0f - 12.0f * m)) * m + m;
        final float bob2 = MathHelper.sin(portal.ticksExisted / (6.0f - 15.0f * m)) * m + m;
        final float alpha = 1.0f - bob;
        scaley -= bob / 4.0f;
        scale -= bob2 / 3.0f;
        this.bindTexture(RenderCultistPortalGreater.portaltex);
        GL11.glPushMatrix();
        GL11.glEnable(3042);
